class ProcessMediaUrlWorker
  ...
  def perform(url, canvas_user_id, assignment_id, submitted_at)
    ...
    if site_and_slug
      ...
      if !previous_entry
        MediaUrl.create(site_and_slug.merge(user_submission_traits).
                        merge({thumbnail_url: thumbnail_url, submitted_at: submitted_at }))
      else
