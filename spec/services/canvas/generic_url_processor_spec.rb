...
RSpec.describe Canvas::GenericUrlProcessor, type: :model do
  ...
  context 'is passed valid JSON for generic url(s)'  do
    ...
    stub_request(:get, "https://localhost:3000/files/259732101/download?download_frd=1").
    ...
    stub_request(:post, "http://localhost:3000/api/v1/courses/1/files").
      to_return(:status => 200, :body => '{"upload_url": "http://localhost:3000/files", "upload_params": {}}', :headers => {'Content-Type': 'application/json'})
    # Stub the actual upload
    stub_request(:post, "http://localhost:3000/files").
      to_return(:status => 302, :body => '', :headers => {'location' => 'http://localhost:3000/confirm'})
    # Stub the confirm request
    stub_request(:post, "http://localhost:3000/confirm").
      to_return(:status => 200, :body => '{"id": 1, "url": "http://localhost:3000/url/to/file"}', :headers => {'Content-Type': 'application/json'})
    # Stub the hiding of the file
    stub_request(:put, "http://localhost:3000/api/v1/files/1").
